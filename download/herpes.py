...
def main(argv):
        ...
        if data_source == "crabs":
				...
				print  "./decode.sh %s %s %s | python uniform.py caida | python herpes.py %s" % (data_source, root_dir, date, out_dir) 
                                os.chdir( "../analyze" ) #ugly,can't change until I figure out a better way
                                ret = os.system( "./decode.sh %s %s %s | python uniform.py caida | python herpes.py %s" % (data_source, root_dir, date, out_dir) )
                                os.chdir( "../download" )
