...
def main(argv):
        ...
        if data_source == "caida":
				...
				print  "./decode.sh %s %s %s | python uniform.py %s | python country.py %s" % (data_source, root_dir, date, data_source, out_dir) 
                                os.chdir( "../analyze" ) #ugly,can't change until I figure out a better way
                                ret = os.system( "./decode.sh %s %s %s | python uniform.py %s | python country.py %s" % (data_source, root_dir, date, data_source, out_dir) )
                                os.chdir( "../download" )
