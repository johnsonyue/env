...
def store(header, graph, data_dir):
	...
	h2 = subprocess.Popen(['gzip', '-c', '-'], stdin=subprocess.PIPE, stdout=fp2)
		...
		h2.stdin.write( "%s %s %s %s %d\n" % (str(in_ip), str(out_ip), is_direct, e.delay, e.num) )
		...
		while i<=len(hop_list)-1:
			...
			if j<len(hop_list):
				...
				if not is_direct:
					...
					node_blank=model_test.Node(blank)
					node_blank.is_blank=True
					ind_blank=graph.add_node(node_blank)
				
				#add edge
				delay=rtt_j-rtt_i
				edge=model_test.Edge(ind_i, ind_j, delay)
