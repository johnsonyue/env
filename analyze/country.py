...
def output(team,date,true_date,monitor,line_dict,data_dir):
	for k in line_dict.keys():
		lines=line_dict[k]
		store(k,team,date,true_date,monitor,lines,data_dir)

def store(cn,team,date,true_date,monitor,lines,data_dir):
	path="%s/%s"%(data_dir,date)
	if ( not os.path.exists(path) ):
		os.makedirs(path)
	
	fn="%s/%s.%s.%s.%s.crabs.gz" % (path,team,true_date,monitor,cn)
	fp=open(fn,'w')
		...
		try:
			...
			#handle header line
				...
				if (team and date and true_date and monitor):
					output(team, date, true_date, monitor, line_dict, data_dir)
					line_dict={}
