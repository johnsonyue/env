...
def merge_edge(file1, file2, file_out):
	...
	while (line1!='' and line2!=''):
		...
		if (cmp < 0):
			...
			edge_type="B" if edge_type1!=edge_type2 else edge_type1
			delay1=float(line1.split(' ')[3])
			delay2=float(line2.split(' ')[3])
			delay=delay1 if delay1 < delay2 else delay2
			line="%s %s %s %s\n" % (edge_field[0], edge_field[1], edge_type, delay)
			fpo.write(line)
			line1=fp1.readline()
			line2=fp2.readline()
		edge_cnt+=1
...
def gunzip(data_dir,file_type):
	file_list = os.popen( "ls %s/*herpes_%s.gz" % (data_dir, file_type) ).readlines()
